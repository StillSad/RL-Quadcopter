...
class DDPG(BaseAgent):
    ...
    def epsilon_greedy_probs(self, state, eps=0.05):
        policy_s = np.ones(self.action_size) * eps / self.action_size
        policy_s[np.argmax(self.Q[state])] = 1 - eps + (eps / self.action_size)
        return policy_s
    ...
    def learn(self,state,action,reward):
        ...
        policy_s = self.epsilon_greedy_probs(state)
        Qsa_next = np.dot(self.Q[state], policy_s)
        self.Q[state][action] = self.update_Q(Qsa, Qsa_next, reward, 0.5, 0.8)
